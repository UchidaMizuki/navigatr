...
corresponding values. The top row shows the menu items (keys on the
left, value summaries on the right). The summaries are `pillar::obj_sum`
outputs, so you can change the printing methods.
